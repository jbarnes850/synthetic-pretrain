...
RUFF_TARGETS="scripts/common.py scripts/materialize_dataset.py scripts/select_targets.py scripts/train.py scripts/evaluate.py scripts/spark_orchestrator.py tools/experiment_log.py tools/result_decider.py"
...
docker run --rm --ipc=host --network=host \
  ...
  -w /workspace \
  "${IMAGE}" \
  bash -lc "python3 -m compileall scripts tools && python3 - <<'PY'
...
docker run --rm --ipc=host --network=host \
  ...
  -w /workspace \
  "${IMAGE}" \
  python3 scripts/materialize_dataset.py --config configs/smoke.yaml --force --validate
...
docker run --rm --ipc=host --network=host \
  ...
  -w /workspace \
  "${IMAGE}" \
  python3 scripts/select_targets.py --config configs/judge_eval.yaml --dry-run-prompts --limit 16 --force
